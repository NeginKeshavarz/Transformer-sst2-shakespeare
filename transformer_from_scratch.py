...
def mmlo_loss(logits, targets, margin=1.0):
    # logits: [B], targets: [B] (float, 0 or 1)
    # margin: hyperparameter, usually 1.0
    pos = logits[targets == 1]
    neg = logits[targets == 0]
    # For positive, want logit > margin
    # For negative, want logit < -margin
    loss_pos = F.relu(margin - pos).mean() if pos.numel() > 0 else 0
    loss_neg = F.relu(margin + neg).mean() if neg.numel() > 0 else 0
...
model = BigramLanguageModel()
glorot_init(model)  # Initialize from scratch with Glorot
m = model.to(device)
